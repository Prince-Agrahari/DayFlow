...
from app.schemas.leave import LeaveActionRequest, LeaveBalanceResponse, LeaveCreateRequest, LeaveResponse
...
def list_leaves(db: DBSession, current_user: CurrentUser, status_filter: LeaveStatus | None = Query(None, alias="status")):
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin access required")
    return [leave_service._to_response(r) for r in leave_service.list_leaves(db, status_filter=status_filter)]

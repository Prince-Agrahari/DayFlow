...
from conftest import make_anomalous_records, make_normal_records
...
def test_anomaly_endpoint_admin(client, admin_headers):
    ...
    item = data["items"][0]
    assert "score" in item and "severity" in item and "reason" in item
...
def test_assistant_missing_context(client, employee_headers):
    response = client.post(
        "/api/ai/assistant",
        headers=employee_headers,
